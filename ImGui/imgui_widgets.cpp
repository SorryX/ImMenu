...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + style.FramePadding + ImVec2(0, label_size.y), window->DC.CursorPos + style.FramePadding + ImVec2(w, label_size.y + 13));
    ...
    value->second = ImLerp(value->second, percent * frame_bb.GetWidth(), 0.140f);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + value->second + 2, frame_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    ...
    window->DrawList->AddText(nullptr, 15.f, frame_bb.Min - ImVec2(0, 2 + label_size.y), ImColor(255, 255, 255, 255), label);
